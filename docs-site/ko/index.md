---
title: 개요
description: 기존 Codex/Claude/Gemini/OpenCode 워크플로를 OpenClaw 스타일 역량으로 강화하는 빠른 진입점.
---

# RexCLI 문서

> 지금 쓰는 CLI 습관은 그대로 유지하고, `codex` / `claude` / `gemini` / `opencode` 위에 OpenClaw 스타일 역량 레이어를 추가합니다.
...
2. 통합 워크플로 레이어 (`codex` / `claude` / `gemini` / `opencode` 그대로 사용)
...
- 입력 조건: Codex/Claude/Gemini/OpenCode 현재 사용 흐름과 끊김 지점.
