...
async function create(projectName, options = {}) {
  ...
  try {
    ...
    if (fs.existsSync(pkgPath)) {
      ...
      pkg.version = '0.1.0';
      // set my name as default author
      pkg.author = pkg.author || 'rexleimo';
      // set license to MIT by default since that's what I use for everything
    ...
    // do an initial commit so there's at least one commit on the branch
    execSync(`git -C ${targetDir} add -A && git -C ${targetDir} commit -m "init: scaffold from ${template} template"`, {
      stdio: 'ignore',
    });
    ...
    // I prefer pnpm over npm - faster and saves disk space
